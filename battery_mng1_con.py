...
def BM_data_fun(state,CDr):
    ...
    if state==0:
        data=numpy.genfromtxt('Cr'+str(CDr)+'.csv', delimiter=',')
    elif state==1:
        data=numpy.genfromtxt('Dr'+str(CDr)+'.csv', delimiter=',')  
    data=data.T
    V_m=data[1]
    SOC_m=data[0]
    
#@autojit
def BM_fun1_arr(a,D,battery_no,state,Cr,lb=0,ub=50,data_extract=0,data_text=" "):
    '''    
...
#        return(10000)
    for i in range(N):
        if R1[i]<cR[0]:
            fitness+=uR*(R1[i]-cR[0])**2
        if R1[i]>cR[1]:
            fitness+=uR*(R1[i]-cR[1])**2
        if R2[i]<cR[0]:
            fitness+=uR*(R2[i]-cR[0])**2
        if R2[i]>cR[1]:
            fitness+=uR*(R2[i]-cR[1])**2
        if C[i]<cC[0]:
            fitness+=uC*(C[i]-cC[0])**2
        if C[i]>cC[1]:
            fitness+=uC*(C[i]-cC[1])**2
        if V_m[i]-V0[i]-cV0>eV01:
            fitness+=uV*(V_m[i]-V0[i]-cV0)**2
        if V_m[i]-V0[i]-cV0<-eV01:
            fitness+=uV*(V_m[i]-V0[i]-cV0)**2
...
def BM_fun2_arr(a,D,battery_no,state,Cr,data_extract=0,lb=0,ub=50,data_text=" "):
    '''    
    ...
    R1=numpy.zeros(shape=(1,N))[0]
    R2=numpy.zeros(shape=(1,N))[0]
    R=numpy.zeros(shape=(1,N))[0]    
    C1=numpy.zeros(shape=(1,N))[0]
    C2=numpy.zeros(shape=(1,N))[0]    
    V0=numpy.zeros(shape=(1,N))[0]
    SOC=numpy.zeros(shape=(1,N))[0]
    ...
    for i in range(N):    
        if state==0:
            ...
            tc=(SOC[N-1]/SOC[i])*tc_total
        R[i]=((a[0]+a[1]*Cr+a[2]*Cr*Cr)*exp(-a[3]*SOC[i])+(a[4]+a[5]*Cr+a[6]*Cr*Cr))
        R1[i]=((a[7]+a[8]*Cr+a[9]*Cr*Cr)*exp(-a[10]*SOC[i])+(a[11]+a[12]*Cr+a[13]*Cr*Cr))
        R2[i]=((a[14]+a[15]*Cr+a[16]*Cr*Cr)*exp(-a[17]*SOC[i])+(a[18]+a[19]*Cr+a[20]*Cr*Cr))
        C1[i]=(-(a[21]+a[22]*Cr+a[23]*Cr*Cr)*exp(-a[24]*SOC[i])+(a[25]+a[26]*Cr+a[27]*Cr*Cr))
        C2[i]=(-(a[28]+a[29]*Cr+a[30]*Cr*Cr)*exp(-a[31]*SOC[i])+(a[32]+a[33]*Cr+a[34]*Cr*Cr))
        V0[i]=((a[35]+a[36]*Cr+a[37]*Cr*Cr)*exp(-a[38]*SOC[i])+(a[39]+a[40]*SOC[i]+a[41]*SOC[i]*SOC[i]+a[42]*(pow(SOC[i],3)))-a[43]*Cr+a[44]*Cr*Cr)
        V1[i]=(V_m[i]-V0[i]-Ic*R[i])/(C1[i]*(1/C1[i]+1/C2[i]))
        V2[i]=(V_m[i]-V0[i]-Ic*R[i])/(C2[i]*(1/C1[i]+1/C2[i]))
        V_c[i]=V0[i]+Ic*R[i]+Ic*R1[i]*(1-exp(-tc/(R1[i]*C1[i])))+Ic*R2[i]*(1-exp(-tc/(R2[i]*C2[i])))+V1[i]*(exp(-tc/(R1[i]*C1[i])))+V2[i]*(exp(-tc/(R2[i]*C2[i])))
        fitness+=abs(V_c[i]-V_m[i])
    for i in range(N):
        if R[i]<cR[0]:
            fitness+=uR*(R[i]-cR[0])**2
        if R[i]>cR[1]:
            fitness+=uR*(R[i]-cR[1])**2
        if R1[i]<cR[0]:
            fitness+=uR*(R1[i]-cR[0])**2
        if R1[i]>cR[1]:
            fitness+=uR*(R1[i]-cR[1])**2
        ...
        if R2[i]>cR[1]:
            fitness+=uR*(R2[i]-cR[1])**2
        if C1[i]<cC[0]:
            fitness+=uC*(C1[i]-cC[0])**2
        if C1[i]>cC[1]:
            fitness+=uC*(C1[i]-cC[1])**2
        if C2[i]<cC[0]:
            fitness+=uC*(C2[i]-cC[0])**2
        if C2[i]>cC[1]:
            fitness+=uC*(C2[i]-cC[1])**2
        if V_m[i]-V0[i]-cV0>eV01:
            fitness+=uV*(V_m[i]-V0[i]-cV0)**2
        if V_m[i]-V0[i]-cV0<-eV01:
            fitness+=uV*(V_m[i]-V0[i]-cV0)**2
